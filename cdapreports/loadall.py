...
loader1 = os.path.abspath(
    os.path.join(tooldir, "..", "lib", "CPTAC-CDAP-Reports", "loader1")
)
mksamp = os.path.abspath(
    os.path.join(tooldir, "..", "lib", "CPTAC-CDAP-Reports", "make_sample")
)
...
for l in h:
    sl = l.strip().split("\t")
    credentials[sl[0]] = dict(username=sl[2], password=sl[3], site=sl[4])
...
for f in dfc:
    ...
    proc = subprocess.Popen(
        args, stdin=None, shell=False, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
