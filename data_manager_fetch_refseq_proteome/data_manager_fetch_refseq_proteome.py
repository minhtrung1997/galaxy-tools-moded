...
def refseq_check(idtag, name, sciname, prefix):
    ...
    idstr = "_".join(
        [_f for _f in map(idstr.get, ["source", "idtag", "datestr"]) if _f]
    )
    desc = " ".join(desc)
...
def refseq_download(idtag, name, sciname, prefix, targetdir):
    ...
    parts = []
    for l in urllib.request.urlopen(listurl):
        sl = l.split()
        if len(sl) != 2:
            continue
    ...
    for part in parts:
        downloader = urllib.request.urlopen(baseurl + "/" + part)
...
def add_data_table_entry(data_manager_dict, table_name, data_table_entry):
    ...
    data_manager_dict["data_tables"][table_name] = data_manager_dict["data_tables"].get(
        "all_proteome", []
    )
...
def main():
    ...
    for l in open(os.path.join(tooldatadir, "all_proteome_species.loc")):
        d = dict(
            list(zip("idtag name taxid refseqsciname refseqprefix".split(), l.split()))
        )
        species[d["idtag"]] = d
